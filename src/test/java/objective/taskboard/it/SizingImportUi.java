...
public class SizingImportUi extends AbstractUiFragment {
    ...
    public SizingImportUi assertWithShowingError(Boolean showingError) {
    	Assert.assertEquals(showingError, hasErrorMessage());
    	return this;
    }

    private Boolean hasErrorMessage() {
    	return sizing.findElements(By.id("errorMessage")).size() > 0;
    }
    
    public void assertShowingCorrectStep(Integer stepNumber) {
    	WebElement activeStepNumber = stepsNumbers.get(stepNumber - 1);
    	waitVisibilityOfElement(activeStepNumber);
    	
    	List<WebElement> step = webDriver.findElements(By.cssSelector(".sizing__step.active[data-step='" + stepNumber + "'"));
    	
    	Assert.assertTrue(activeStepNumber.getAttribute("class").contains("active"));
    	Assert.assertEquals(1, step.size());
    }
    ...
    class SizingStepOne {
        ...
        public SizingStepOne() {
            ...
            WebElement selectProjectElement = step.findElement(By.id("projectkey"));
        ...
        public SizingStepOne assertHasProjects() {
        	Assert.assertTrue(selectProject.getOptions().size() > 0);
            return this;
        }
    ...
    class SizingStepTwo extends SizingMiddleStep {
        ...
        public void assertColumnValueEquals(Integer columnIndex, String columnValue, Boolean isEquals) {
            String valueToCompare = requiredColumnsOfAdvancedMapping.get(columnIndex).getAttribute("value");
            Assert.assertTrue(valueToCompare.equals(columnValue) == isEquals);  
        }
    ...
    class SizingStepFour {
        ...
        public void assertCloseSizingImport() {
            returnToTaskboard();
        }
